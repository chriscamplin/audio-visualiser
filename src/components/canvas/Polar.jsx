    ...
    <>
      <mesh
        ...
        onPointerMove={(e) => {
          if (trackPointer) {
            lerpTarget.uv.set(
              MathUtils.clamp(e.uv.x, 0.3, 0.7),
              MathUtils.clamp(e.uv.y, 0.3, 0.7)
            )
            lerpTarget.tube.set(
              MathUtils.clamp(e.point.x, size * -0.2, size * 0.2),
              0.5,
              MathUtils.clamp(e.point.z, 5 * -0.2, 5 * 0.2)
            )
          }
        }}
        onPointerOut={() => {
      ...
      {showTexture && (
        <Html fullscreen style={{ pointerEvents: 'none' }}>
          <img
            className='debugTexture'
            src={matProps.debug ? DEBUG_TEXTURE : CAUSTICS_TEXTURE}
            alt='debug'
          />
        </Html>
      )}
