  ...
  const onBeforeCompile = useMemo(
    () => (shader) => {
      ...
      shader.vertexShader = /* glsl */ `
            ...
            const float animationSeconds = 10.;
            ...
            vec3 computeVert(float angle,float H){
              float sineVal = sin(PI2 * (time - 0.75) / animationSeconds) / 2.0 + 0.5;

              float snd = amplitude;

              float STEP=sineVal*.7*frequency;
    ...
    ${shader.vertexShader}`
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        vec3 A_col=vec3(1,1,1)*vNorm*rot2*rot;
